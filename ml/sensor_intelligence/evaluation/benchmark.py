        ...
        "resources": [
            {
                ...
                "publisher_checksums_verified": item.get("publisher_checksums_verified", 0),
                "nested_archives": [
                    {
                        "path": nested["path"],
...
def _participant_error_report(
    ...
    return {
        ...
        "participant_macro_summary": {
            ...
            "participants_with_evaluated_windows": len(maes),
            "participants_without_evaluated_windows": len(per_participant) - len(maes),
...
def _activity_report(rows: list[dict[str, Any]], accepted: np.ndarray) -> dict[str, dict[str, Any]]:
    report: dict[str, dict[str, Any]] = {}
    activities = sorted({int(row["activity_id"]) for row in rows if row["activity_id"] is not None})
...
def _serialize_model(model: Pipeline, feature_names: list[str], threshold: float) -> dict[str, Any]:
...
def _bidmc_respiration_report(dataset_root: Path, participants: tuple[str, ...]) -> dict[str, Any]:
    ...
    for participant in participants:
        ...
        for start in range(0, ppg.values.size - width + 1, stride):
            ...
            participant_values[participant]["agreement"].append(float(ppg_result.components_agree))
    ...
    return {
        ...
        "ppg_multimodulation_fusion": regression_metrics(reference_values, ppg_estimates),
        ...
        "impedance_respiration_sanity": regression_metrics(reference_values, impedance_estimates),
    }
...
def run_ppg_dalia_benchmark(
    ...
    model = Pipeline(
        steps=[
            ...
            (
                "classifier",
                LogisticRegression(class_weight="balanced", max_iter=2_000, random_state=20260829),
            ),
        ]
    )
    ...
    report: dict[str, Any] = {
        ...
        "quality_gate": {
            ...
            "validation": _classification_metrics(validation_y, validation_probability, threshold),
...
def run_bidmc_external_benchmark(
    ...
    report = {
        ...
        "held_out_test_respiration_rate": _bidmc_respiration_report(dataset_root, split.test),
    }
    output_root.mkdir(parents=True, exist_ok=True)
...
def run_wrist_exercise_external_benchmark(
    ...
    report = {
        ...
        "adapter_version": "wrist-unit-contract-v2",
        "motion_feature_unit": "g",
        "supersedes": "wrist reports without adapter_version used incompatible acceleration units",
        "non_diagnostic": True,
        "benchmark_integrity": (
            "The PPG-DaLiA quality model, its parameters, and acceptance threshold are loaded "
        ...
        "frozen_quality_gate": _frozen_quality_gate_report([
            {**row, "participant_id": str(row["participant_id"]).split("_", maxsplit=1)[0]}
            for row in rows
        ]),
        "scientific_limitations": (
...
def run_capnobase_external_respiration_benchmark(
    ...
    for case_id in capnobase.case_ids(dataset_root):
        ...
        for start in starts:
            ...
            reference_mask = (record.reference_times_seconds >= start_seconds) & (
    ...
    case_within_5 = [
        float(item["all_windows"]["within_5_bpm_fraction"]) for item in case_reports.values()
    ]
    report = {
        "schema_version": "1.0.0",
        "generated_at": datetime.now(UTC).isoformat(),
