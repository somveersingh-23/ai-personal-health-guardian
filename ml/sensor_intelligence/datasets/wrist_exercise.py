...
def _acceleration_in_g(values: np.ndarray, units: list[str]) -> np.ndarray:
    """Convert WFDB physical channels to the PPG-DaLiA model's g feature contract."""
    scales = []
    for unit in units:
        if unit in {"g", "G"}:
            scales.append(1.0)
        elif unit in {"ms^-2", "m/s^2", "m/s2", "m/s/s"}:
            scales.append(1.0 / STANDARD_GRAVITY_M_S2)
        else:
            raise ValueError(f"unsupported acceleration unit: {unit!r}")
    return values * np.asarray(scales)


def _signal_indices(names: list[str], tokens: tuple[str, ...]) -> list[int]:
    return [
        index for index, name in enumerate(names) if any(token in name.lower() for token in tokens)
    ]
...
def load_record(dataset_root: Path, record_id: str) -> ResearchRecord:
    ...
    if len(ppg_indices) != 1 or len(acceleration_indices) != 3:
        raise ValueError(
    ...
    retained_samples = _common_valid_length(values[:, [ppg_indices[0], *acceleration_indices]])
    ppg = values[:retained_samples, ppg_indices[0]]
    acceleration = np.linalg.norm(
        _acceleration_in_g(
            values[:retained_samples, acceleration_indices],
            [raw.units[index] for index in acceleration_indices],
        ),
        axis=1,
    )
    ...
    return ResearchRecord(
        ...
        metadata={
            ...
            "acceleration_source_units": [raw.units[index] for index in acceleration_indices],
