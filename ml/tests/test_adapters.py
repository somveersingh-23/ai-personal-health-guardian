...
def test_ptt_ppg_adapter_preserves_publisher_channel_order(
    ...
    csv_root.mkdir()
    pd.DataFrame({"record": ["s1_sit"], "spo2_start": [98], "spo2_end": [97]}).to_csv(
        csv_root / "subjects_info.csv", index=False
    )
...
def test_wrist_motion_is_converted_to_the_training_unit():
    samples = np.array([[9.80665, 0.0, 0.0]])
    converted = wrist_exercise._acceleration_in_g(samples, ["ms^-2"] * 3)
    assert np.linalg.norm(converted, axis=1)[0] == pytest.approx(1.0)
    assert wrist_exercise._acceleration_in_g(np.ones((1, 3)), ["g"] * 3).tolist() == [
        [1.0, 1.0, 1.0]
    ]
    with pytest.raises(ValueError, match="unsupported acceleration unit"):
        wrist_exercise._acceleration_in_g(samples, ["arbitrary"] * 3)
