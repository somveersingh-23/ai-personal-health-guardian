...
from app.services.sensors.transactions import member2_write
from app.models.member2.collection import SensorCollectionState
from app.services.sensors.quality_service import _freshness_score
...
def _event_fields(event: HealthEventCreate) -> dict[str, object]:
    return {
        ...
        "consent_receipt_id": str(event.consent_receipt_id)
        if event.consent_receipt_id
        else None,
...
def event_from_orm(row: HealthEvent, now: datetime | None = None) -> HealthEventCreate:
    ...
    age = max(0, int(((now or datetime.now(UTC)) - reading_reference_time(event)).total_seconds()))
    return event.model_copy(update={
        "freshness_status": derive_freshness_status(age),
        "data_freshness_seconds": age,
        "quality_vector": event.quality_vector.model_copy(update={"freshness_score": _freshness_score(age)}),
    })
...
async def _find_existing(
    session: AsyncSession, event: HealthEventCreate
) -> HealthEvent | None:
    ...
    if len(rows) > 1:
        raise ValueError("event_id and source identity refer to different records")
    row = rows[0] if rows else None
    if row is not None and (
        row.user_id != event.user_id
        or row.source != event.source.value
        or row.metric != event.metric.value
        or row.temporal_type != event.temporal_type.value
        or (row.data_origin_package, row.source_record_type, row.source_record_id)
        != (event.data_origin_package, event.source_record_type, event.source_record_id)
    ):
        raise ValueError("event source identity, owner and metric are immutable")
    return row


async def _find_tombstone(
    session: AsyncSession, event: HealthEventCreate
) -> SourceTombstone | None:
...
async def persist_batch(
    ...
    normalized = [
        await enrich_event_with_device_capability(session, event)
        for event in normalized
    ]
    if any(
        event.record_integrity_status == IntegrityStatus.REJECTED
        for event in normalized
    ):
        raise ValueError("batch contains a rejected record; no records were persisted")
    if any(
        event.quality_vector.decision == QualityDecision.REJECTED
        for event in normalized
    ):
    ...
    try:
        for event in normalized:
            ...
            if tombstone is not None:
                ...
                if (
                    tombstone.consent_receipt_id is not None
                    or modified_at is None
                    or deleted_at is None
                    or modified_at <= deleted_at
                ):
                    duplicate_count += 1
                    continue
            ...
            if existing is not None:
                ...
                continue
            row = HealthEvent(
                **_event_fields(event), ingested_at=received, updated_at=received
            )
            session.add(row)
            rows.append(row)
            inserted += 1
...
async def fetch_events_for_window(
    ...
    result = await session.execute(
        ...
        .where(
            ...
            or_(
                and_(
                    HealthEvent.observed_at >= window_start,
                    HealthEvent.observed_at < window_end,
                ),
                and_(
                    HealthEvent.start_at < window_end, HealthEvent.end_at > window_start
                ),
            ),
        )
    ...
    rows = list(result.scalars().all())
    if len(rows) > 1000:
        raise ValueError("fusion window exceeds 1000 records; request a smaller window")
...
async def list_active_events(
    ...
    """Read-only M2 contract for downstream dashboard and AI consumers.

    It excludes deleted observations but preserves provenance and quality fields,
    so consuming features cannot mistake a low-quality source value for truth.
    """
    clauses = [
        HealthEvent.user_id == user_id,
        HealthEvent.lifecycle_status != "deleted",
    ...
    if metrics:
        clauses.append(HealthEvent.metric.in_([metric.value for metric in metrics]))
    result = await session.execute(
        select(HealthEvent)
        .where(*clauses)
        .order_by(HealthEvent.updated_at.desc(), HealthEvent.id.desc())
        .limit(limit)
    )
    return [event_response_from_orm(row) for row in result.scalars().all()]


async def export_active_events_page(
    session: AsyncSession,
    user_id: int,
    after_id: int | None,
    limit: int,
) -> tuple[list[HealthEventResponse], int | None, bool]:
    """Return a stable ascending page for privacy/data-portability export."""
    ...
    if after_id is not None:
        clauses.append(HealthEvent.id > after_id)
    result = await session.execute(
        select(HealthEvent)
        .where(*clauses)
        .order_by(HealthEvent.id.asc())
        .limit(limit + 1)
    )
    rows = list(result.scalars().all())
    ...
    return [event_response_from_orm(row) for row in page], next_after_id, has_more
...
async def delete_source_records(
    ...
    if deletion_time > datetime.now(UTC):
    ...
    eligible_ids = set(request.source_record_ids) - {
        row.source_record_id
        for row in rows
        if row.source_last_modified_at is not None
        and as_utc(row.source_last_modified_at) > deletion_time
    }
    ...
    tombstones = list(existing_tombstones.scalars().all())
    tombstoned = {row.source_record_id for row in tombstones}
    for row in tombstones:
        if row.source_record_id in eligible_ids and as_utc(row.deleted_at) < deletion_time:
            row.deleted_at = deletion_time
    rows_by_source_id = {row.source_record_id: row for row in rows}
    for source_record_id in request.source_record_ids:
        if source_record_id in tombstoned or source_record_id not in eligible_ids:
            continue
    ...
    result = await session.execute(
        ...
        .where(
            ...
            HealthEvent.source == request.source.value,
            HealthEvent.source_record_type == request.source_record_type,
            HealthEvent.source_record_id.in_(eligible_ids),
        )
...
async def reconcile_source_records(
    ...
    conditions = [
        HealthEvent.user_id == user_id,
        HealthEvent.updated_at <= request.window_end,
        HealthEvent.source == request.source.value,
        HealthEvent.source_record_type == request.source_record_type,
        ...
        or_(
            ...
            and_(
                HealthEvent.start_at < request.window_end,
                HealthEvent.end_at > request.window_start,
            ),
        ),
    ]
    if request.source_record_ids:
        conditions.append(
            HealthEvent.source_record_id.not_in(request.source_record_ids)
        )
...
async def upsert_device(
    session: AsyncSession, user_id: int, device: DeviceInfo
) -> DeviceRegistry:
    result = await session.execute(
...
@member2_write(allow_paused=True)
async def purge_member2_data(session: AsyncSession, user_id: int) -> dict[str, int]:
    """Delete all M2-owned data for one profile in a single transaction.

    M1's health profile is deliberately not touched. Counts are returned so the
    caller can present an auditable completion result without retaining values.
    """

    state = await session.get(SensorCollectionState, user_id)
    if state is None:
        session.add(SensorCollectionState(user_id=user_id, paused=True))
    else:
        state.paused = True
    reconciliation_ids = select(ReconciliationSession.session_id).where(
    ...
    targets = (
        (
            ...
            delete(ReconciliationRecord).where(
                ReconciliationRecord.session_id.in_(reconciliation_ids)
            ),
        ),
        (
            "reconciliation_sessions",
            delete(ReconciliationSession).where(
                ReconciliationSession.user_id == user_id
            ),
        ),
        ("health_events", delete(HealthEvent).where(HealthEvent.user_id == user_id)),
        (
            "source_tombstones",
            delete(SourceTombstone).where(SourceTombstone.user_id == user_id),
        ),
        (
            "device_capabilities",
            delete(DeviceCapability).where(DeviceCapability.user_id == user_id),
        ),
        (
            "device_registries",
            delete(DeviceRegistry).where(DeviceRegistry.user_id == user_id),
        ),
        (
            "health_connect_sync_states",
            delete(HealthConnectSyncState).where(
                HealthConnectSyncState.user_id == user_id
            ),
        ),
        (
            "consent_receipts",
            delete(ConsentReceipt).where(ConsentReceipt.user_id == user_id),
        ),
        (
            "sensor_ingestion_audits",
            delete(SensorIngestionAudit).where(SensorIngestionAudit.user_id == user_id),
        ),
    )
    counts: dict[str, int] = {}
    try:
        for name, statement in targets:
            result = await session.execute(
