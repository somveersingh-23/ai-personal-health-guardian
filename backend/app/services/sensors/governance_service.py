...
async def create_consent_receipt(
    ...
    existing = await session.execute(
        select(ConsentReceipt).where(
            ConsentReceipt.receipt_id == str(receipt.receipt_id)
        )
    )
    ...
    if row is not None:
        ...
        incoming = {
            **values,
            "consented_at": _as_utc(receipt.consented_at).isoformat(),
            "expires_at": _as_utc(receipt.expires_at).isoformat()
...
async def validate_event_consents(
    ...
    for event in v3_events:
        ...
        if (
            receipt.status != ConsentStatus.ACTIVE.value
            or (expires_at and expires_at <= now)
            or _as_utc(receipt.consented_at) > now
        ):
    ...
    if request.delete_linked_observations:
        ...
        existing = await session.execute(
            select(
                ...
                SourceTombstone.source_record_type,
                SourceTombstone.source_record_id,
            ).where(
                SourceTombstone.user_id == user_id,
                SourceTombstone.consent_receipt_id == str(receipt_id),
            )
        )
        tombstoned = set(existing.all())
        for row in rows:
            if (
                row.source_record_id is None
                or (row.source, row.source_record_type, row.source_record_id)
                in tombstoned
            ):
                continue
            session.add(
