...
async def upsert_device_capabilities(
    ...
    for capability in request.capabilities:
        ...
        if capability.reference_method or capability.validation_protocol_version:
            raise ValueError(
                "validation evidence is managed by the trusted server registry"
            )
    ...
    for capability in request.capabilities:
        ...
        values["recording_methods_json"] = [
            item.value for item in capability.recording_methods
        ]
        ...
        if row is None:
            row = DeviceCapability(
                user_id=user_id, device_id=request.device_id, **values
            )
            session.add(row)
        else:
            if (
                row.support_status != "experimental"
                or row.calibration_status != "unverified"
                or row.reference_method
                or row.validation_protocol_version
            ):
                raise ValueError(
                    "client cannot replace a server-managed device capability"
                )
...
async def enrich_event_with_device_capability(
    ...
    if capability.calibration_status == "expired" or (
        capability.calibration_status == "valid"
        and capability.calibration_valid_until is not None
        and _as_utc(capability.calibration_valid_until) <= datetime.now(UTC)
    ):
        calibration_confidence = 0.0
        decision = QualityDecision.REJECTED
        reasons.append("device_calibration_expired")
    if (
        capability.source_type != event.source.value
        or capability.canonical_unit_ucum != event.canonical_unit_ucum
        or (
            capability.recording_methods_json
            and event.recording_method.value not in capability.recording_methods_json
        )
    ):
        decision = QualityDecision.REJECTED
        reasons.append("device_capability_contract_mismatch")
