...
def assess_record_integrity(
    ...
    if reading.source_last_modified_at is not None and (
        reading.source_last_modified_at.astimezone(UTC) - received_utc
    ).total_seconds() > MAX_CLOCK_SKEW_SECONDS:
        raise ValueError("source revision time exceeds future clock-skew tolerance")
