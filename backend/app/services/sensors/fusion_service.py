...
def _event_in_window(event: HealthEventCreate, start: datetime, end: datetime) -> bool:
    ...
    if event.temporal_type == TemporalType.SERIES:
        return any(start <= sample.observed_at < end for sample in event.samples)
    return (
...
def _quality(event: HealthEventCreate) -> float:
    ...
    optional = (
        ...
        None
        if vector.motion_artifact_score is None
        else 1.0 - vector.motion_artifact_score,
...
def _missing_quality_dimensions(events: list[HealthEventCreate]) -> list[str]:
    ...
    return sorted(
        ...
        if all(getter(event) is None for event in events)
    )


def _source_contradiction(
    metric: MetricType, events: list[HealthEventCreate], start: datetime, end: datetime
) -> str | None:
    ...
    for event in events:
        if event.value is not None:
            ...
            values_by_source[_source_key(event)].extend(
                sample.value
                for sample in event.samples
                if start <= sample.observed_at < end
            )
    means = [
        sum(values) / len(values) for values in values_by_source.values() if values
    ]
...
def _coverage(events: list[HealthEventCreate], start: datetime, end: datetime) -> float:
    intervals = [
        ...
        and event.end_at is not None
        and _event_in_window(event, start, end)
    ]
...
def _select_cumulative_source(
    ...
    return max(
        grouped.values(),
        key=lambda group: (
            _coverage(group, start, end),
            sum(_quality(item) for item in group) / len(group),
        ),
    )
...
def _aggregate_mean(
    ...
    for event in candidates:
        ...
        if (
            event.temporal_type == TemporalType.INSTANT
            ...
            samples = [
                sample for sample in event.samples if start <= sample.observed_at < end
            ]
            if samples:
...
def _aggregate_sleep_duration(
    ...
    for event in selected:
        usable_stages = [
            stage for stage in event.stages if stage.stage in SLEEPING_STAGES
        ]
        if event.stages:
            intervals.extend(
            ...
            raise ValueError("sleep stages are required to estimate time asleep")
    seconds = _merge_intervals(intervals)
    return (
        seconds / 60.0,
        sum(len(event.stages) for event in selected),
        seconds,
        selected,
    )
...
def fuse_events(
    ...
    for metric in requested_metrics:
        candidates = [
            ...
            and _quality(event) > 0
            and _event_in_window(event, window_start, window_end)
        ]
        if not candidates:
            missing.append(metric)
            continue
        if len({event.unit for event in candidates}) != 1:
            contradictions.append(f"{metric.value}:incompatible_units")
            missing.append(metric)
            continue
        if metric == MetricType.SLEEP_DURATION:
            candidates = [event for event in candidates if event.stages]
            if not candidates:
                missing.append(metric)
                continue
        if metric == MetricType.SKIN_TEMPERATURE and candidates[0].unit == "degC_delta":
            # Deltas from different baselines/sites are not a common absolute scale.
            if any(
                (
                    event.metadata.get("baseline_celsius") is not None
                    and type(event.metadata["baseline_celsius"]) not in (int, float)
                )
                or (
                    event.metadata.get("measurement_location") is not None
                    and type(event.metadata["measurement_location"]) not in (int, str)
                )
                for event in candidates
            ):
                contradictions.append("skin_temperature:invalid_baseline_context")
                missing.append(metric)
                continue
            contexts = {
                (
                    event.device_id,
                    event.data_origin_package,
                    event.metadata.get("baseline_celsius"),
                    event.metadata.get("measurement_location"),
                )
                for event in candidates
            }
            if len(contexts) > 1 or (
                len(candidates) > 1
                and any(
                    event.metadata.get("baseline_celsius") is None
                    for event in candidates
                )
            ):
                contradictions.append(
                    "skin_temperature:incompatible_or_unknown_baselines"
                )
                missing.append(metric)
                continue
        contradiction = _source_contradiction(
        ...
        if method == AggregationMethod.QUALITY_WEIGHTED_MEAN:
            ...
            source_records = _select_cumulative_source(
                candidates, window_start, window_end
            )
            ordered = sorted(source_records, key=lambda item: item.start_at)
            partial = any(
                item.start_at < window_start or item.end_at > window_end
                for item in ordered
            )
            overlapping = any(
            ...
            if partial or overlapping:
                contradictions.append(f"{metric.value}:requires_source_aggregation")
                missing.append(metric)
                continue
            value, sample_count, coverage, selected = _aggregate_sum(
                candidates, window_start, window_end
            )
        ...
        features.append(
            FusedMetric(
                ...
                selected_source_keys=[
                    f"{package}:{device}"
                    for package, device in sorted(
                        {_source_key(item) for item in selected}
                    )
                ],
    ...
    composite = (
        sum(feature.quality_weight for feature in features) / len(features)
        if features
        else 0.0
    )
    ...
    return MultimodalFeatureVector(
        ...
        provenance={
            ...
            "devices": sorted(
                {event.device_id for event in used_events if event.device_id}
            ),
            "data_origin_packages": sorted(
                {event.data_origin_package for event in used_events}
            ),
...
def fuse_baseline_evidence(
    ...
    strength = (
        sum(item.evidence_weight for item in evidence) / len(evidence)
        if evidence
        else 0.0
    )
    return MultimodalEvidenceVector(
