...
from app.schemas.member3.notifications import NotificationChannel, NotificationCreateRequest
...
def test_expired_tampered_and_missing_tokens_are_rejected(monkeypatch):
    ...
    # Invalid signature
    assert client.get(endpoint, headers={"Authorization": f"Bearer {_token(invalid_sig=True)}"}).status_code == 401
    # Tampered string
    assert client.get(endpoint, headers={"Authorization": f"Bearer {_token()}tampered"}).status_code == 401
    # No sub claim
    assert client.get(endpoint, headers={"Authorization": f"Bearer {_token(no_sub=True)}"}).status_code == 401
...
def test_cross_user_access_rejected_across_endpoints(monkeypatch):
    ...
    assert client.get("/api/v1/member3/insights?user_id=user-b", headers=auth_headers).status_code == 403
    assert client.get("/api/v1/member3/alerts?user_id=user-b", headers=auth_headers).status_code == 403
    assert client.get("/api/v1/member3/notifications?user_id=user-b", headers=auth_headers).status_code == 403
    assert client.get("/api/v1/member3/caregivers?user_id=user-b", headers=auth_headers).status_code == 403
    assert client.get("/api/v1/member3/conversations?user_id=user-b", headers=auth_headers).status_code == 403
    assert client.get("/api/v1/member3/emergency/workflows?user_id=user-b", headers=auth_headers).status_code == 403
...
def test_prompt_injection_does_not_override_safety_action():
    ...
    context = StructuredPromptContext(
        ...
        evidence=(
            EvidenceSummary(
                metric="heart_rate", current_value=165.0, baseline_value=72.0,
                unit="bpm", direction="elevated", confidence=0.98, signal_quality=0.95,
            ),
        ),
    ...
    assert "emergency" in explanation.lower() or "urgent" in explanation.lower() or "safety" in explanation.lower()
    assert "not a medical diagnosis" in explanation.lower() or "not medical advice" in explanation.lower() or "disclaimer" in explanation.lower()
...
def test_complete_data_deletion_purges_all_member3_records():
    ...
    baseline = BaselineResult(
        user_id="user-purge", event_id="event-purge-1", metric="heart_rate",
        baseline=70.0, current=95.0, unit="bpm", deviation_score=2.5,
        status="above_normal", confidence=0.9, occurred_at=datetime.now(timezone.utc),
    )
    sensors = SensorIntelligenceResult(
        event_id="event-purge-1", user_id="user-purge", signal_quality=0.95, fusion_confidence=0.9,
    )
    req = UpstreamGuardianAdapter().build(baseline, sensors)
    services.guardian.process(req)

    # 2. Add Caregiver
    services.caregivers.create(CaregiverLinkCreate(
        user_id="user-purge", caregiver_user_ref="caregiver-doc", relationship_label="Physician"
    ))
...
def test_guardian_process_replay_is_idempotent():
    ...
    baseline = BaselineResult(
        user_id="user-idempotent", event_id="event-idem-1", metric="heart_rate",
        baseline=70.0, current=80.0, unit="bpm", deviation_score=1.1,
        status="above_normal", confidence=0.9, occurred_at=datetime.now(timezone.utc),
    )
    sensors = SensorIntelligenceResult(
        event_id="event-idem-1", user_id="user-idempotent", signal_quality=0.95, fusion_confidence=0.9,
    )
    req = UpstreamGuardianAdapter().build(baseline, sensors)
...
def test_webhook_secret_authenticates_delivery_receipt(monkeypatch):
    ...
    notif = app.state.member3_services.notifications.create(
        NotificationCreateRequest(
            user_id="user-w", source_event_id="ev-w", title="T", body="B",
            priority=AlertPriority.HIGH, channels=[NotificationChannel.IN_APP],
